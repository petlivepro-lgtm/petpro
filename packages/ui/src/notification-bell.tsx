...
import { relativeTime, unreadCount, type NotificationItem } from "./notifications";
import { usePush, type UsePushOptions } from "./use-push";

/**
 * Ícone injetado pelo app. Tipado estruturalmente para o pacote não depender de
 * `lucide-react`, que é dependência dos apps e não do design system — qualquer
 * componente que aceite `className` serve.
 */
...
/**
 * O painel é popover ancorado no sino no desktop e folha inferior no celular.
 * A escolha precisa existir em JS (e não só em classes) porque o popover é
 * posicionado por medida — mesmo motivo pelo qual DatePicker e Select calculam
 * `top/left` à mão.
 */
...
export type NotificationBellProps = {
  /** Lista já sincronizada pelo app (realtime fica do lado de quem tem o client). */
  items: NotificationItem[];
  /** Ícone por `kind`. Kinds ausentes caem em `fallbackIcon`. */
  icons: Record<string, IconComponent>;
  /** Sino, e ícone de qualquer notificação sem mapeamento. */
  bellIcon: IconComponent;
  bellOffIcon: IconComponent;
  bellRingIcon: IconComponent;
  closeIcon: IconComponent;
  /** Clique numa notificação: o app decide navegar e marcar como lida. */
  onOpenItem: (item: NotificationItem) => void | Promise<void>;
  onMarkAllRead: () => void | Promise<void>;
  /** Largura a partir da qual vira popover ancorado (768 = md, 1024 = lg). */
  desktopFrom?: number;
  emptyHint?: string;
  /** Texto do botão de ativar — muda entre "painel" e "app". */
...
export function NotificationBell({
  ...
  const isNarrow = useIsNarrow(desktopFrom);
  ...
  // Trava o scroll só na folha inferior; no desktop o popover acompanha o
  // scroll em vez de bloqueá-lo.
  ...
  const updatePos = React.useCallback(() => {
    ...
    const margin = 8;
    // Alinhado à DIREITA do sino: ele fica na borda direita da barra, e um
  ...
  React.useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      // Não fecha um drawer/dialog que esteja em volta.
      e.stopPropagation();
      setOpen(false);
    };
  ...
  async function handleItem(item: NotificationItem) {
    setOpen(false);
    await onOpenItem(item);
  }

  function toggle() {
    // Zera a posição ao abrir para o painel ser medido antes de aparecer —
    // sem isso o "virar para cima" calcularia com a altura errada.
    if (!open) setPos(null);
    setOpen((o) => !o);
  }
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain">
          {items.length === 0 ? (
            <div className="px-5 py-10 text-center">
              ...
              {emptyHint && (
                <p className="mt-1 text-xs text-gray-neutral">{emptyHint}</p>
              )}
            </div>
          ) : (
 ...
 * configurações, porque é onde a pessoa percebe que existe — e o navegador só
 * aceita o pedido de permissão a partir de um clique dela.
 */
function PushFooter({
  options,
  bellOffIcon: BellOff,
  ...
  const { state, busy, error, enable, disable } = usePush(options);
  ...
  if (state === "unsupported") {
    return (
      <p className={cn(base, "text-xs text-gray-neutral")}>
        ...
        Este navegador não recebe avisos. No iPhone, adicione à tela de início
        primeiro.
      </p>
    );
  }
  ...
  return (
    <div className="border-t border-graphite/10">
      ...
      >
        ...
        {busy
          ? "Ativando..."
          : (enableLabel ?? "Avisar neste aparelho mesmo com o app fechado")}
      </button>
      {error && <p className="px-4 pb-3 text-xs text-danger">{error}</p>}
    </div>
  );
