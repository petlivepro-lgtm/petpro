...
// Camada de leitura do boletim de comportamento do pet.
//
...
/**
 * Média e nº de avaliações por pet, indexado por pet_id. Uma query resolve a
 * lista inteira — a view `pet_behavior_summary` já agrega no banco.
 */
...
/** Categorias do boletim configuradas pelo petshop (tenant.settings.behavior). */
export async function fetchBehaviorCategories(
  ...
  const { data } = await supabase
    ...
    .eq("id", tenantId)
    .maybeSingle();
  const settings = (data?.settings ?? {}) as { behavior?: unknown };
  const parsed = behaviorConfigSchema.safeParse(settings.behavior);
