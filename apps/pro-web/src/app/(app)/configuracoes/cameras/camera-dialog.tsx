...
export function CameraDialog({ camera }: { camera?: CameraRow }) {
  ...
  const [state, formAction, pending] = useActionState<FormState, FormData>(upsertCamera, {
    ok: false,
  });
      ...
      >
        <form action={formAction} className="space-y-4">
          ...
          <p className="-mt-2 text-xs text-gray-neutral">
            Veja o IP no app Tapo (Configurações do Dispositivo) e fixe-o no roteador
            (reserva DHCP) para ele não mudar.
          </p>

          <div>
            <Label htmlFor="stream_path">Qualidade da transmissão</Label>
            <Select id="stream_path" name="stream_path" defaultValue={camera?.stream_path ?? "stream1"}>
          ...
          <div className="rounded-xl border border-graphite/10 bg-graphite/[0.03] p-3">
            <p className="text-xs text-gray-neutral">
              Crie a <strong>Conta da Câmera</strong> no app Tapo: Configurações do
              Dispositivo → Configurações Avançadas → Conta da Câmera (6 a 32 caracteres,
              diferente da conta TP-Link) e informe-a abaixo.
            </p>
          </div>

          <div>
            <Label htmlFor="username">Usuário da Conta da Câmera *</Label>
            <Input id="username" name="username" required defaultValue={camera?.username} />
          </div>
